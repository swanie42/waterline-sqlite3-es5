...
const WaterlineError = require('waterline-errors');
const WaterlineCursor = require('waterline-cursor');
...
const Adapter = {
  ...
  buildSchema(connection, collections) {
    return _.chain(collections).map((model, modelName) => {
  ...
  describe(connectionName, tableName, cb) {
    ...
    return Promise.all([cxn.knex.raw(`pragma table_info("${tableName}")`), cxn.knex.raw(`pragma index_list("${tableName}")`)]).then(([tableInfo = [], indexList = []]) => {
      return Promise.all(indexList.map(index => {
        return cxn.knex.raw(`pragma index_info("${index.name}")`).then(([indexInfo = {}]) => {
  ...
  drop(connectionName, tableName, relations = [], cb = relations) {
    ...
    return cxn.knex.schema.dropTableIfExists(tableName).then(result => {
      _.isFunction(cb) && cb();
    }).catch(AdapterError.wrap(cb));
  },
  ...
  addAttribute(connectionName, tableName, attributeName, definition, cb) {
    ...
    return cxn.knex.schema.table(tableName, table => {
      let newColumn = Util.toKnexColumn(table, attributeName, definition);
  ...
  create(connectionName, tableName, record, cb) {
    ...
    return cxn.knex.transaction(txn => {
      return txn.insert(Util.castRecord(record)).into(tableName).then(([rowid]) => {
  ...
  find(connectionName, tableName, options, cb) {
    ...
    if (options.where) {
      Object.keys(options.where).forEach(column => {
        if (Array.isArray(options.where[column]) && options.where[column].length === 0) delete options.where[column];
      });

      if (Object.keys(options.where).length === 0) options.where = null;
    }
  ...
  update(connectionName, tableName, options, data, cb) {
    ...
    return cxn.knex.transaction(txn => {
      return new Promise((resolve, reject) => {
        ...
        return txn.select('rowid').from(tableName).whereRaw(txn.raw(Util.toKnexRawQuery(whereClause), values));
      }).then(rows => {
  ...
  destroy(connectionName, tableName, options, cb) {
    ...
    return this.find(connectionName, tableName, options).then(_found => {
      found = _found;
