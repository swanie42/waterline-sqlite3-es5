...
const CriteriaParser = require('waterline-sequel/sequel/lib/criteriaProcessor');
...
const Util = {
  ...
  toKnexColumn(table, _name, attrDefinition) {
    let attr = _.isObject(attrDefinition) ? attrDefinition : {
      type: attrDefinition
    };
  ...
  transformTableInfo(tableInfo, indexes) {
    return _.chain(tableInfo).map(column => {
      let index = _.findWhere(indexes, {
        cid: column.cid
      });
